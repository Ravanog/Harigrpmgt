...
HELP_TXT = os.environ.get(
    ...
    "• `/rules` — View group guidelines & rules (Auto-deletes in 10s)\n"
    "• `/setrules [text]` — Set custom group rules *(Admins)*\n\n"
    "👋 **Welcome & Channels:**\n"
    "• `/setwelcome [text]` — Set custom welcome message *(Admins)*\n"
    "• `/setchannel1 [link]` — Set or update Channel 1 button link *(Admins)*\n"
    "• `/setchannel2 [link]` — Set or update Channel 2 button link *(Admins)*\n\n"
    "⚡ **Admin Moderation Tools:**\n"
    "• `/ban` — Permanently ban replied user *(Admins)*\n"
    "• `/mute` — Restrict replied user from chatting *(Admins)*\n"
    "• `/unrestrict` — Restore user permissions *(Admins)*\n"
    "• `/pin` — Pin replied message with alert *(Admins)*\n"
    "• `/purge` — Bulk delete messages in chat *(Admins)*\n\n"
    "📊 **Analytics & System:**\n"
    "• `/users` — View group stats & user analytics\n"
    "• `/groups` — List all connected groups *(Owner only)*\n"
    "• `/start` — Launch the bot start panel"
)
